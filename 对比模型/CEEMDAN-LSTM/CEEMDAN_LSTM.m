...
%         load(filename)
    ...
    tic;
%         load('beijing.mat')   % 加载数据
%         data =beijing;      % 加载的数据必须是向量，不能是矩阵
%         load('shanghai.mat')   % 加载数据
%         data =shanghai;      % 加载的数据必须是向量，不能是矩阵
        load('ceemdan_imf.mat')   % 加载数据
        data =ceemdan_imf(10,:);      % 加载的数据必须是向量，不能是矩阵
    %% -------------------------------------------------------------------------
    % 原始数据划分（用于对比）
